...
resource "aws_security_group" "bastion_ssh_all_sg_concord" {
    
    name            = "bastion_ssh_all_sg"
    description     = "Allow SSH from Anywhere"
    vpc_id          = aws_vpc.vpcconcord.id #module.site.output_vpc_id 
...
output "output_bastion_ssh" {
  value = aws_security_group.bastion_ssh_all_sg_concord.id
}
...
resource "aws_security_group" "bastion_ssh_from_nat" {
    ...
    ingress {
     ...
     protocol        = "tcp"
     security_groups = [
                        aws_security_group.bastion_ssh_all_sg_concord.id,
